do
local Arash = 109722284  --put your id here(BOT OWNER ID)

local function setrank(msg, name, value) -- setrank function
  local hash = nil
  if msg.to.type == 'chat' then
    hash = 'User rank:'..msg.to.id..':variables'
  end
  if hash then
  ...
  if result.username then
   ...
   Username = '----'
    ...
    local text = 'Full name : '..(result.first_name or '')..' '..(result.last_name or '')..'\n'
               ..'User name: '..Username..'\n'
               ..'ID : '..result.id..'\n\n'
	local hash = 'User rank:'..extra.chat2..':variables'
	local value = redis:hget(hash, result.id)
    if not value then
	 if result.id == tonumber(Arash) then
	   text = text..'User Rank : Executive Admin \n\n'
	  elseif is_admin2(result.id) then
	   text = text..'User Rank : Admin \n\n'
	  elseif is_owner2(result.id, extra.chat2) then
	   text = text..'User Rank : Leader \n\n'
	  elseif is_momod2(result.id, extra.chat2) then
	    text = text..'User Rank : Moderator \n\n'
      else
	    text = text..'User Rank : Member \n\n'
	 end
   else
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
  ...
  local text = 'Full name : '..(result.from.first_name or '')..' '..(result.from.last_name or '')..'\n'
               ..'Username : '..Username..'\n'
               ..'ID : '..result.from.id..'\n\n'
	local hash = 'User rank:'..result.to.id..':variables'
