...
local function run(msg, matches)
  if matches[1]:lower() == 'swatchtg' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /swatchtg ")
    return about
  end 
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "SwatchTG" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "^[!/]([Ss]tatslist)$",
    "^[!/]([Ss]tats) (group) (%d+)",
    "^[!/]([Ss]tats) (SwatchTG)",-- Put everything you like :)
		"^[!/]([Ss][Ww][Aa][Tt][Cc][Hh][Tt][Gg])"-- Put everything you like :)
    }, 
  run = run
}
